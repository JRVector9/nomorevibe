...
export function thumbnailPresentation(src:string|null|undefined){
 ...
 return {kind,label:LABELS[kind],identity:['site_icon','github_avatar','default'].includes(kind)||(kind==='repository_image'&&(Math.max(width,height)<=256||(width/height>=0.85&&width/height<=1.18))),width,height};
}
