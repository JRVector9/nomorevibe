 ...
 *
 * 예산 110초(scripts/worker.ts) 안에서 돈다. 게이트웨이가 붐비면 한 건이 45초까지 가는데, 틱이
 * 짧으면 "남은 시간에 못 끝낼 호출은 시작하지 않는다"는 규칙에 걸려 뒤쪽이 통째로 놀았다.
 * 잠금이 있어 틱이 겹치지 않으므로, 긴 틱은 곧 쉬지 않고 도는 것과 같다.
 */
const CONCURRENT = 4;
