...
클라이언트가 조작할 수 없다. 같은 방문자의 같은 제품 클릭은 10분 창으로 묶는다 — 한 번
...
제품당 10분에 한 번만 세진다. 신원이 아니라 "같은 브라우저인가"만 본다.
...
순위의 `유효 클릭`은 `/go/<slug>`를 통해 외부 서비스로 이동한 클릭 중 봇을 제외하고, 같은
방문자·제품 조합은 10분에 한 번만 센 값이다. 시즌 점수는 `유효 클릭 × 순위 반영률`이다. 직전
시즌 상위 제품에는 관리자가 정한 소프트 쿨다운을 적용하고, 시즌이 지날수록 100%까지 회복시켜
같은 제품이 계속 상단을 독점하지 않게 한다. 24시간 변동률은 인접한 두 구간을 비교하며, 이전
구간 클릭이 설정된 최소값보다 작으면 과장된 퍼센트 대신 `신규`로 표시한다.
