...
type Search = { state?: string | string[]; page?: string | string[]; cause?: string | string[]; ai?: string | string[]; focus?: string | string[] };
...
export default async function ReviewPage({ searchParams }: { searchParams: Promise<Search> }) {
  ...
  const [takedowns, causes, decisions] = await Promise.all([pendingTakedowns(), reviewQueueCauses(settings), reviewQueueAiDecisions()]);
  ...
  const ids = causeIds && aiIds ? causeIds.filter((id) => aiIds.includes(id)) : causeIds ?? aiIds;
  const filtered = Boolean(cause || ai);
  ...
  const query = (next: Partial<Record<'state' | 'cause' | 'ai' | 'page', string | number | undefined>>) => {
    const merged = { state: filtered ? undefined : state === 'pending' ? undefined : state, cause: cause || undefined, ai: ai || undefined, ...next };
    ...
    <main className="flex flex-col gap-2.5 pb-10 pt-6">
      ...
      {entries.length === 0 ? (
        ...
        <>
          ...
          <ReviewConsole key={`${page}:${state}:${cause}:${ai}`} entries={entries} reasons={REVIEW_REJECT_REASONS} bulkFormId={BULK_FORM} focus={focus} />
        </>
      )}
