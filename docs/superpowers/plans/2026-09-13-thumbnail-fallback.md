...
- [x] Run unit/integration/typecheck/lint/build; independently review code using codex CLI if available; fix actionable findings. Commit own files only, migrate directDB, apply frozen production cohort; report counts while processing.
- [x] Deploy web2/crawler/publisher/maintenance/scheduler; verify fresh source and job executions plus desktop/mobile images. Record final source counts, exclusions and unresolved failures in operations report and CODEX_HANDOFF.md.
