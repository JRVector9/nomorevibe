...
- `npm run job <name>`은 명시적 요청을 남긴 뒤 한 tick을 실행한다. 잠금/쿼터 대기가 있으면 요청은 남는다.
...
감시기는 아래 초기 상한을 넘으면 SIGTERM을 보낸다. 단순 유휴/쿼터 대기 자체는 장애가 아니다.
...
agent scan은 20초 제한을 사용하지만 전체 tick은 여러 bounded 작업과 DB 처리를 포함한다.
...
```sh
node --import tsx scripts/measure-worker-capacity.ts --origin=http://127.0.0.1:3200 --paths=/,/ranking --rps=2 --duration-seconds=60 --max-inflight=4
```

출력 JSON에는 실제 달성 RPS, HTTP 계열/429/네트워크 오류, p50/p95/p99, 건너뛴 슬롯·최대 동시 요청이 있다.
응답 본문은 수집하지 않는다. 측정 전후 CPU/RSS/DB 대기와 함께 보관하고 병목이 확인된 항목만 개선한다.
